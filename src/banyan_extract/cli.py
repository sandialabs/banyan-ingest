    ...
    if args.is_input_dir:
        ...
        if backend == "auto":
            for filepath, basename in zip(file_paths, basenames):
                ...
                # Process single file
                output = processor.process_document(filepath, re_run=args.re_run)
                if args.checkpointing:
                    output.save_output(output_directory, basename)
        else:
            # Use the selected processor for all files
            outputs = document_processor.process_batch_documents(file_paths, use_checkpointing=args.checkpointing, draw_bboxes=args.draw_bboxes, output_dir=output_directory, re_run=args.re_run)
        ...
        filename = args.input_file
        outputs = document_processor.process_document(filename, re_run=args.re_run)
